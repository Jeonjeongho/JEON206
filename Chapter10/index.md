...
* 키는 반드시 객체여야 한다.
* WeakMap의 키는 [가비지 콜렉션](https://goo.gl/RKJmPN)에 포함될 수 있다.
* WeakMap의 키들은 열거형이 아니다.(즉, 키 목록을 제공해 주는 메서드가 없다는 것이다.)
...
셋은 값들의 집합이다. 입력된 순서에따라 저장된 요소를 반복처리할 수 있다. Set은 중복된 값을 허용하지 않는다. 따라서 특정 값은 Set내에서 하나만 존재 하게 된다.
